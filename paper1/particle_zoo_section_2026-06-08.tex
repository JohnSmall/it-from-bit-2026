...
%  UPDATED 2026-07-12: two currency additions to sec:sedenion-announcement
%  -- (1) the Klein-four-group sentence: both the K3 classes and the
%  sedenion sectors are (Z2 x Z2, S3 = Aut) canonically, so the shadow
%  statement's abstract half is secured and the intertwiner is what
%  remains; (2) notebook M's boundary condition on the near-conservation
%  bonus: the bare cross-algebra vertex is maximal, the dressing carries
%  the suppression. Staging unchanged (K3 record, sedenion candidate).
%  UPDATED 2026-07-10 (the discovery narrative, per JS): new subsection
%  sec:sedenion-announcement after the triality paragraph -- the K3
%  rough edge diagnosed (smuggled rung-distinguishability vs the
%  canonical orbits' permutation symmetry), the circuit forcing the
%  extra wire, the triality promotion Aut(O) = G2 -> Aut(S) = G2 x S3
%  (brown1967generalized, RIS delivered), generations as the S3-orbit,
%  sterile as the invariant sector, the shadow-theorem relation stated
%  at candidate status with the K3 derivation remaining the derivation
%  of record; op:filatov-extension gains the route clause; the
%  circuit-as-critic methodological coda per JS's framing.
%  RE-AUDITED 2026-07-06 (final pass, twelfth stop, light leg): the
%  "next section makes quantitative" adjacency broke under the reorder
%  (confinement intervenes) -- retargeted to sec:masses; three
%  consumer-pointers added, problem content untouched everywhere:
%  op:filatov-extension gains the frame's certification-gap reading,
%  op:chirality the frame's parity setting, op:sterile-gravity the
%  spacetime dark-matter back-stitch ("the two statements are one
%  problem"); header umlaut normalised, file pure ASCII; the deep
%  audit otherwise stands.
% =====================================================================
...
%      is next worked).
%
%  AUDITED 2026-07-04: DM candidacy demoted to open (op:sterile-gravity
%  now carries it; seesaw role only asserted); sterile mass tied to the
%  3.6 TeV matching scale; GHZ-side gap folded into op:filatov-extension
%  (companion clause) and flagged at first use, honouring the register
%  section's pointer; sterile spin-1/2 status routed through the
%  register ladder with its owed bridges; Bell-shadow wording made
%  precise; GHZ/sterile disambiguation + two-neutrals contrast added.
...
exactly three of them, and finally a fourth class that couples to no gauge force. The spine of the argument
...
exhibits for the leptons. Carrying the resolution machinery from the W triangle, where it is
derived, to the GHZ class, whose pairs are not pairwise entangled, is itself a structural step; it
is folded into the foundational open problem below. They appear as a left-handed doublet, the up- and down-type quark, in
...
right-handed and the other left-handed \autocite{filatov2024towards}. This is the Filatov
...
A W-class channel has all three of its qubit pairs entangled: tracing any one qubit leaves the bipartite \emph{Bell shadow}, a two-qubit state whose dominant
component is a genuine Bell pair (Corollary in Appendix~\ref{app:w-proof}). Extending the
...
so that the Filatov--Auzinsh opposite-handedness theorem \autocite{filatov2024towards} applies to
every pair independently. A companion clause travels with it: establish that the resolution-class
structure so obtained governs the GHZ class as well --- whose pairs are not entangled, hence outside
the two-sphere theorem's hypothesis --- as the quark doublet and generations of this section
require. The generation count of this section rests on this extension; the combinatorics that
follow are rigorous once it is granted. (The certification reading of the same gap ---
state-certified leptons, specification-certified quarks --- is drawn at \S\ref{sec:frame}. A
candidate dissolution of the companion clause's burden --- the generation label as sedenion-level
data, discovered when the mixing circuit demanded its wire --- is recorded at
\S\ref{sec:sedenion-announcement} and developed in the follow-up.)
\end{openproblem}
...
The record of how that corroboration sharpened belongs in the paper, because it is the paper's
method demonstrating itself. The $K_3$ argument above was this programme's first account of the
generation count, and it carried a rough edge its authors could feel: the reading requires knowing
\emph{which} qubit is the odd one out and what rung it occupies, while the canonical orbits of
\S\ref{sec:canonical-orbits} are permutation-symmetric --- the state treats the three qubits as
interchangeable, and the label's rung-identity is imported from the tower architecture rather than
derived. Nothing was inconsistent; something was not quite right.

The diagnosis arrived from an unexpected direction. Extending the Cabibbo computation of
Appendix~\ref{app:qiskit-ckm} to the full three-generation mixing circuit
(Appendix~\ref{app:hw-ckm}) proved impossible on the register alone: the construction demanded an
additional qubit to carry the generation index. Initially this was an embarrassment --- an
unlicensed wire with no rung in the architecture. Thought about more deeply, it was the answer.
The four-qubit space is exactly the complexified sedenions,
$(\mathbb{C}^{2})^{\otimes 4} = \mathbb{C}^{16} = \mathbb{C} \otimes \mathbb{S}$, one
Cayley--Dickson rung past the register --- and the doubling changes the symmetry inventory in
precisely the needed way. The octonions' automorphism group $G_{2}$ contains no $S_{3}$:
$\mathrm{Spin}(8)$ triality, the ``deeper reason'' of the preceding paragraph, is invisible to the
octonions' own automorphisms. The sedenions' automorphism group is $G_{2} \times S_{3}$
\autocite{brown1967generalized}: the doubling \emph{promotes} triality to a manifest symmetry,
acting by permuting three octonion subalgebras whose triple intersection is a shared quaternionic
core \autocite{gillardgresnigt2019three}. On that reading the three generations are the
$S_{3}$-orbit of octonion subalgebras --- the count a theorem of the algebra rather than a
frustration argument --- the sterile class is the $S_{3}$-invariant sector, with nothing to
permute matching its lack of a frustrated pair; the shared core is why every generation carries
identical electroweak quantum numbers, a fact the $K_{3}$ device never explained; and the extra
qubit stops needing apology: it is the wire on which triality finally acts, and the mixing circuit
could not have been built without it because the index never lived downstairs.

The two accounts are not rivals; the candidate relation is that the first is the shadow of the
second --- the $S_{3}$ action, projected through the selected complex direction, reproducing the
frustration classes on the register, with the three Fano lines of the preceding paragraph as the
correspondence's published half. One half of the statement is already secured at the abstract
level: the four $K_{3}$ classes form a Klein four-group under class addition --- the symmetric
class the identity, the product of two frustrated classes the third --- with
$S_{3} = \mathrm{Aut}(\mathbb{Z}_{2} \times \mathbb{Z}_{2})$ permuting the three non-identity
elements, and the sedenion sector structure carries the same
$\mathbb{Z}_{2} \times \mathbb{Z}_{2}$ grading with the same automorphism action; what remains
of the shadow statement is therefore the physical intertwiner, not the group isomorphism. Proving that shadow statement, verifying the algebraic inputs at
citation grade, and formalising the bonus the sedenion reading offers and the $K_{3}$ device did
not --- generation number as an approximately conserved charge, violated only in the zero-divisor
sector, which would make the near-diagonality of the mixing matrices a mechanism rather than a
mystery (a structural scan has since fixed that mechanism's boundary condition: the bare
cross-algebra vertex mixes \emph{maximally}, so the near-diagonality must be carried entirely by
the dressing --- the suppression, not the coupling, is the object to derive). The dressing
programme's first exact results land on the same side: the tag-recorded excursion shift is
generation-universal and the induced mixing vanishes identically --- an exact cancellation
arranged by the $S_3$ twist --- so the near-diagonality is protected at second order by the
same structure that carries the generations --- is the business of the follow-up paper, and until it is done the derivation of record
in this paper remains the $K_{3}$ argument at the status Open Problem~\ref{op:filatov-extension}
prices. But the methodological point stands on its own and we state it plainly: this is how a
...
A disambiguation is owed here, because the phrase ``all three the same'' invites a specific
misreading. The sameness is of \emph{handedness} --- the chirality labels $\chi \in \{L, R\}$ on
the three spheres' coordinate frames, the Filatov data --- not of basis kets. The two all-same
assignments, all-$L$ and all-$R$, form one class because the global flip exchanges particle and
antiparticle; they are classical alternatives, not branches of a superposition. In particular the
sterile class is not the GHZ state wearing other clothes: $\lvert 000\rangle + \lvert 111\rangle$
is one coherent state whose two \emph{basis components} are superposed, maximally
tripartite-entangled, and --- by Proposition~\ref{prop:ghz} --- resident on the colour-triplet side
of the split, which is to say a quark. The sterile is the opposite pole of the machinery: a frame
labelling with no opposite-handedness pair, hence no entangled edge for the Filatov representation
to draw, colour-singlet, and blind to every gauge gate. And the two neutral leptons are neutral for
different reasons: the active neutrino's neutrality is a \emph{cancellation} --- $T_3$ against
$\tfrac12 Y$ in exactly the combination the photon reads (\S\ref{sec:charges}) --- while the
sterile's is an \emph{absence}: no winding, no pair, nothing to cancel.
...
active neutrino, which is why one standard objection to any dark-matter reading of such a state ---
that parity restoration at high energy should regenerate it --- would not apply: there is nothing
for it to be the partner of.
...
that register structure as \S\ref{sec:register} derives it --- established there for the
pairwise-entangled triangle and extended to the remaining classes with the owed bridges that
section records; a-chirality is an additional internal property the
...
The framework reads it as a sterile neutrino, with a computability-scale Majorana mass driving a
seesaw; whether it can also serve as a dark-matter candidate is exactly what the tension recorded
below (Open Problem~\ref{op:sterile-gravity}) leaves open, and only the seesaw role is asserted
here. On the count that two such a-chiral modes exist, the seesaw yields a massless lightest active
neutrino, $m_1 = 0$, normal mass ordering, and two sterile states near the matching scale of
\S\ref{sec:charges}, roughly $3.6\ \mathrm{TeV}$. The count of a-chiral modes is the weakest link in this part of the
...
insight. We said above that the channel couples to no gauge force; yet it carries a Majorana mass, and mass,
...
tension the framework's identification of gravity with measurement makes unavoidable.
 The spacetime section's dark-matter prospect names the same condition from the
metric side --- participation in loop-counting prior to, and independent of, gauge participation
(\S\ref{sec:space-of-computations}) --- and the two statements are one problem.
\end{openproblem}
...
structure alone. The structural setting --- parity sensitivity as the job description of a
frame-acting force --- is drawn at \S\ref{sec:frame}.
\end{openproblem}
...
qualitatively --- that mass grows with algebraic depth --- \S\ref{sec:masses} makes quantitative,
