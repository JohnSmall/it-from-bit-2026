...
%  Labels in this section: op:traceless, op:anomalies.
%
...
$\mathbf{3}$ of $SU(3)$: this is colour, and a quark is a channel whose $C$-qubit is GHZ-entangled
with its partners in a hadron. A lepton channel --- a canonical W orbit --- is a colour singlet:
...
the W-marginal spectrum but by the resolution classes of \S\ref{sec:generations}. A weak-isospin
...
The status of the trace condition should be stated plainly, because it is the load-bearing
assumption of this section. A trace part would be a phase rotation proportional to the identity on
$\mathbb{O}$ --- an overall phase of the whole register --- and in this framework the overall phase
is the observer-private, self-referential phase that the Born-rule construction renders
unobservable (\S\ref{sec:register}); the gauge-observable $U(1)$ must therefore be the traceless
part. We regard this as a well-motivated structural assumption rather than a theorem, and record it
as a named open problem.

\begin{openproblem}[Tracelessness of $U(1)_Y$]\label{op:traceless}
Show that the gauge kinetic term derived from the Hopf holonomy forces the gauge-observable $U(1)$
to be the traceless part of the phase rotation on $\mathbb{O}$, promoting the global-phase argument
above from motivation to theorem.
\end{openproblem}
...
$U(1)_Y^3$, the mixed $SU(3)^2$--$U(1)_Y$, and the gravitational anomaly --- involve the
right-handed singlet hypercharges below; they are satisfied by the resulting assignments, as in the
Standard Model, but their structural status is open.

\begin{openproblem}[Higher anomalies from traces]\label{op:anomalies}
Derive the cubic $U(1)_Y^3$, the mixed $SU(3)^2$--$U(1)_Y$ and the gravitational anomaly
cancellations structurally, each as a trace condition on the appropriate sector of the register,
rather than as a check on the resulting assignments.
\end{openproblem}

The right-handed singlets complete the table.\label{sec:rh-singlets} They have $T_3 = 0$, and the
chirality flip that produces them preserves electric charge (the \emph{Zitterbewegung} zigzag,
adopted here and made circuit-level at the Higgs vertex of \S\ref{sec:interactions}), so their
