...
New references go in as a dated BibLaTeX batch under `bib/`, imported into
Zotero. Write the batch as `.bib`: an entry key is a citation key, and Better
BibTeX's importer keeps it and pins it, so the key you write is the key the
export will carry. RIS cannot do this --- Zotero's RIS importer has no
citation-key field and discards the `ID` --- and the `.ris` files in `bib/`
are the batches imported before that was understood. Keys are
`author+year+keyword`, inherited from the Mendeley library the collections
were built from. `bib/README.md` has the details, including the two ways an
import can still go wrong.
...
├── paper1/, paper2/, notes/, notebooks/, scripts/, bib/ ...
